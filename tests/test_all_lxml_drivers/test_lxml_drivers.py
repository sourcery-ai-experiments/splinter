...
def test_cant_switch_to_frame(browser):
...
def test_forward_to_none_page(request, browser):
...
def test_cant_mouseover(browser):
...
def test_cant_mouseout(browser):
...
def test_finding_all_links_by_non_ascii_text(browser):
...
def test_links_with_nested_tags_xpath(browser):
    browser.visit(EXAMPLE_APP)

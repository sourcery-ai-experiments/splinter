...
def test_element_query_should_raises_when_element_first_doest_exists(browser):
    browser.visit(EXAMPLE_APP)
    with pytest.raises(ElementDoesNotExist):
        browser.find_by_css(".element-that-dont-exists").first


def test_element_list_raises_when_element_last_does_not_exists(browser):
    browser.visit(EXAMPLE_APP)
    with pytest.raises(ElementDoesNotExist):
        browser.find_by_css(".element-that-dont-exists").last


def test_element_list_raises_when_element_does_not_exists(browser):
    browser.visit(EXAMPLE_APP)
    with pytest.raises(ElementDoesNotExist):
        browser.find_by_css(".element-that-dont-exists")[2]


def test_element_list_raises_with_unicode_query(browser):
    browser.visit(EXAMPLE_APP)
    with pytest.raises(ElementDoesNotExist):
        browser.find_by_css(".element[title=título]").last


def test_element_list_contains_right_information_and_raises_right_exception(browser):
